    ...
    <div class="my_card user-card">
      <el-form class="search_bar flex flex-wrap" label-width="70px" label-position="left">
        ...
        <el-form-item label="账号状态">
          <el-select placeholder="请选择" collapse-tags v-model="queryData.status" clearable>
            <el-option label="有效" :value="1" :key="1"> </el-option>
            <el-option label="禁用" :value="0" :key="0"> </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="账号类型">
          <el-select placeholder="请选择" collapse-tags v-model="queryData.type" clearable>
      ...
      <el-table :data="state.dataSource" style="width: 100%" v-loading="state.loading" element-loading-text="拼命加载中"
        ref="tableRef" border>
        ...
        <el-table-column label="操作">
          <template v-slot="scope">
            ...
            <el-button type="text" @click="changeStatus(scope.row)">{{ scope.row.user_status === 1 ? '禁用' : '启用' }}
            </el-button>
            <el-button type="text" style="color: #f56c6c" v-if="state.isSuperAdmin" @click="deleteUser(scope.row.id)">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="page_nation" v-if="totalNum > 0">
        <el-pagination background layout="prev, pager, next ,sizes,total " :page-sizes="[10, 20]"
          :page-size="queryData.limit" :current-page="queryData.page" :total="totalNum"
          @current-change="handleCurrentChange" @size-change="handleSizeChange">
        </el-pagination>
      </div>
    </div>
    <el-dialog title="用户权限" v-model="state.showDrawer" destroy-on-close>
  ...
  ElMessageBox.confirm(
    ...
    .then(async () => {
      ...
      if (res.code === 200) {
        ...
        state.dataSource = state.dataSource.filter((item: UserInfo) => item.id !== id)
      }
    })
    .catch(() => {
    })
